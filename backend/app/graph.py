...
from app.excel_service import (
    FileValidationError, infer_columns, inspect_xlsx, profile_quality, read_sheet,
)
...
def build_analysis_graph(
    ...
    def validate_file(state: AnalysisState) -> dict[str, Any]:
        ...
        try:
            ...
            if path.suffix.lower() != ".xlsx" or not path.exists() or path.stat().st_size > MAX_FILE_SIZE:
                raise FileValidationError("الملف لا يطابق سياسة الرفع الآمن.")
        except (ValueError, OSError, FileValidationError) as exc:
            return {"status": "failed", "error": str(exc), "stage": "validate_file", "progress": 4, "trace": ["فشل التحقق الأمني من الملف."]}
        return {"status": "running", "stage": "validate_file", "progress": 8, "trace": ["اجتاز الملف فحوص الامتداد والحجم والمسار الآمن."]}

    def inspect_workbook(state: AnalysisState) -> dict[str, Any]:
        try:
            info = inspect_xlsx(Path(state["file_path"]), state["original_name"], state["mime_type"], state["file_id"])
        except FileValidationError as exc:
            return {"status": "failed", "error": str(exc), "stage": "inspect_workbook", "trace": ["تعذر فحص بنية المصنف."]}
        return {"workbook_info": info.model_dump(mode="json"), "stage": "inspect_workbook", "progress": 16, "trace": [f"تم اكتشاف {len(info.sheets)} ورقة عمل."]}

    def detect_tables(state: AnalysisState) -> dict[str, Any]:
        frame = read_sheet(Path(state["file_path"]), state["sheet_name"])
        if frame.empty or not len(frame.columns):
            return {"status": "failed", "error": "ورقة العمل فارغة.", "table_info": {"analyzable": False}, "stage": "detect_tables", "trace": ["لم تُكتشف بيانات قابلة للتحليل."]}
        return {"table_info": {"analyzable": True, "rows": len(frame), "columns": len(frame.columns)}, "stage": "detect_tables", "progress": 23, "trace": [f"تم اكتشاف جدول يحوي {len(frame)} صفًا و{len(frame.columns)} عمودًا."]}

    def infer_semantics(state: AnalysisState) -> dict[str, Any]:
        frame = read_sheet(Path(state["file_path"]), state["sheet_name"])
        columns = infer_columns(frame, state.get("column_mapping", []))
        return {"columns": [c.model_dump() for c in columns], "stage": "infer_semantics", "progress": 31, "trace": ["اكتملت قراءة أنواع الأعمدة وأدوارها الدلالية."]}

    def detect_ambiguities(state: AnalysisState) -> dict[str, Any]:
        ambiguous = [column for column in state["columns"] if column["ambiguous"]]
        return {"ambiguous_columns": ambiguous, "stage": "detect_ambiguities", "progress": 38, "trace": [f"تم رصد {len(ambiguous)} عمود غامض."]}

    def request_user_clarification(state: AnalysisState) -> dict[str, Any]:
        answer = interrupt({
            "kind": "column_clarification",
            "title": "نحتاج توضيحًا قصيرًا",
            "question": "ما الدور الصحيح للأعمدة التالية؟",
        ...
        return {"column_mapping": mapping, "ambiguous_columns": [], "stage": "request_user_clarification", "progress": 43, "trace": ["استؤنف التحليل بعد حفظ توضيح المستخدم."]}

    def profile_dataset(state: AnalysisState) -> dict[str, Any]:
        frame = read_sheet(Path(state["file_path"]), state["sheet_name"])
        columns = infer_columns(frame, state.get("column_mapping", {}))
        quality = profile_quality(frame, columns)
        return {"columns": [c.model_dump() for c in columns], "quality": quality.model_dump(), "stage": "profile_dataset", "progress": 53, "trace": [f"اكتمل تقرير الجودة بدرجة {quality.score}."]}
    ...
    def create_analysis_plan(state: AnalysisState) -> dict[str, Any]:
        try:
            ...
            return {
                ...
                "progress": 62, "trace": ["أوقف النظام التنفيذ لأن مزود النموذج غير جاهز أو أعاد خطة غير آمنة."],
            }
    ...
    def execute_analysis(state: AnalysisState) -> dict[str, Any]:
        ...
        try:
            ...
            dashboard = dashboard_builder(frame, state["columns"], quality, state["sheet_name"], state.get("analysis_plan"))
            if isinstance(dashboard, DashboardSpec): dashboard = dashboard.model_dump(mode="json")
            return {"dashboard": dashboard, "iteration": iteration, "validation_errors": [], "stage": "execute_analysis", "progress": 73, "trace": [f"نُفذت الحسابات البرمجية بالمحاولة {iteration}/{state['max_iterations']}."]}
        except Exception as exc:
            return {"iteration": iteration, "validation_errors": [str(exc)], "stage": "execute_analysis", "trace": [f"تعذر تنفيذ التحليل في المحاولة {iteration}."]}

    def validate_results(state: AnalysisState) -> dict[str, Any]:
        if state.get("validation_errors"):
            return {"stage": "validate_results", "trace": ["رُفضت النتائج قبل التحقق البنيوي."]}
        try:
            dashboard = DashboardSpec.model_validate(state["dashboard"])
            assert_numeric_provenance([i.text for i in dashboard.detailed_insights], dashboard.computed_results)
            return {"dashboard": dashboard.model_dump(mode="json"), "validation_errors": [], "stage": "validate_results", "progress": 81, "trace": ["نجحت مطابقة النتائج الرقمية مع سجل الحسابات."]}
        except (ValidationError, ValueError) as exc:
            return {"validation_errors": [str(exc)], "stage": "validate_results", "trace": ["فشل تحقق DashboardSpec أو توثيق الأرقام."]}

    def generate_dashboard_spec(state: AnalysisState) -> dict[str, Any]:
        dashboard = DashboardSpec.model_validate(state["dashboard"])
        return {"dashboard": dashboard.model_dump(mode="json"), "stage": "generate_dashboard_spec", "progress": 88, "trace": ["تم تثبيت مواصفة DashboardSpec المتحققة للواجهة."]}
    ...
    def handle_failure(state: AnalysisState) -> dict[str, Any]:
        return {"status": "failed", "stage": "handle_failure", "progress": 100, "error": state.get("error", "تعذر إكمال التحليل."), "trace": ["أوقف النظام المسار بأمان دون تنفيذ حسابات غير موثقة."]}
