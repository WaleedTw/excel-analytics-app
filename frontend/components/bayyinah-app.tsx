...
function CleaningNotice({ audit, proposal = false }: { audit: CleaningAuditView; proposal?: boolean }) {
  ...
  const strategyLabel = { derived:t("علاقة محسوبة", "Calculated relationship"), sequential:t("تسلسل معرّف", "Identifier sequence"), mean:t("المتوسط", "Mean"), median:t("الوسيط", "Median"), label:t("وسم واضح", "Explicit label"), manual:t("إدخال يدوي", "Manual entry"), retained:t("مراجعة مطلوبة", "Review required") } as const;
  return <section className={`cleaning-notice ${missingTotal ? "has-missing" : "clean"}`}>
    ...
    {actions.length>0&&<div className="cleaning-actions">{actions.map((action)=><article className={action.strategy === "retained" ? "retained" : "resolved"} key={action.column}><div><b>{action.column}</b><small><span dir="ltr">{enNumber(action.count)}</span> {t("قيمة ناقصة", "missing values")}{action.source_rows.length ? <> · {t("صفوف Excel", "Excel rows")}: <span dir="ltr">{action.source_rows.map(enNumber).join(", ")}</span></> : null}</small></div><strong>{strategyLabel[action.strategy]}</strong><p>{action.explanation}{action.fill_value!=null&&action.strategy!=="label"?<> {t("القيمة/القاعدة المستخدمة", "Applied value/rule")}: <b dir="ltr">{typeof action.fill_value === "number" ? enNumber(action.fill_value) : action.fill_value}</b>.</>:null}</p></article>)}</div>}
    <footer>{audit.formula_calculations>0&&<span><Activity/> {t("احتُسبت", "Calculated")} <b dir="ltr">{enNumber(audit.formula_calculations)}</b> {t("صيغة صفية آمنة", "safe row formulas")}</span>}{audit.removed_duplicate_rows.length>0&&<span><X/> {t("أزيل", "Removed")} <b dir="ltr">{enNumber(audit.removed_duplicate_rows.length)}</b> {t("صف مكرر مطابق", "exact duplicate rows")}</span>}{audit.excluded_empty_columns.length>0&&<span><X/> {t("استُبعدت العواميد الفارغة", "Excluded empty columns")}: {audit.excluded_empty_columns.join(", ")}</span>}{Object.keys(audit.remaining_missing_values).length>0&&<span className="needs-review"><CircleAlert/> {t("بقيت للمراجعة", "Still requires review")}: {Object.entries(audit.remaining_missing_values).map(([column,count])=>`${column} (${enNumber(count)})`).join(", ")}</span>}</footer>
  </section>;
}
